...
from agent.prompts.plan_generation_prompt import PLAN_GENERATION_PROMPT
...
def generate_comprehensive_plans(state: PlanGenerationState) -> PlanGenerationState:
    ...
    if not docs or not combined_content.strip():
        error_msg = "Plan generation requires extracted document content; none available"
    ...
    # Set error if no plans were generated
    error = None
    if not generated_plans:
        error = "Failed to generate any management plans"
...
def create_plan_asset_spec(state: PlanGenerationState) -> Dict[str, Any]:
    ...
    if not state.generated_plans or state.error:
        return {}
