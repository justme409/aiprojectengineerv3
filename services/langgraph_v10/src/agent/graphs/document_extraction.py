...
async def document_extraction_node(state: ExtractionState) -> Dict[str, Any]:
    ...
    # Set error if all documents failed
    error = None
    if len(failed_docs) > 0 and len(documents) == 0:
        error = f"All {len(failed_docs)} documents failed to extract. First error: {failed_docs[0]['error']}"
    elif len(failed_docs) > 0:
        error = f"{len(failed_docs)} documents failed to extract"
